...
- 微服务架构版本：可以分别启动网关、认证、流程、项目管理、代码生成等多个服务。
...
![首页展示](https://s21.ax1x.com/2025/07/22/pVGCHAS.png)
![项目概览页](https://s21.ax1x.com/2025/07/22/pVGCbtg.png)
![任务编辑页](https://s21.ax1x.com/2025/07/22/pVGCOpj.png)

![](https://s21.ax1x.com/2025/07/22/pVGCX1s.png)

### 2.3、代码展示

![LokHub代码展示](https://gitee.com/seaflows/picgo-bed-diagram/raw/master/image/202507221527360.png)
...
```
git clone git@github.com:Wwwsea/lokhub.git
```
...
- GitHub 地址：[GitHub - Wwwsea/lokhub: 鸟类信息展示平台](https://github.com/Wwwsea/lokhub)
...
推荐使用 [Navicat | 免费下载 Navicat Premium Lite](https://www.navicat.com.cn/download/navicat-premium-lite)这款免费图形化数据库管理工具。
...
②、如果是 Windows 用户，可以直接双击 redis-server.exe 启动 Redis。
...
①、如果是 macOS 用户，可以直接在终端输入`sh startup.sh -m standalone`启动 Nacos。

②、如果是 Windows 用户，可以直接双击 startup.cmd 启动 Nacos。

启动成功后访问 http://localhost:8848/nacos 即可看到 Nacos 控制台。默认用户名密码都是 nacos。

* 4、启动 SkyWalking 分布式链路追踪（非必须）

参考手册：[极简入门 | Apache SkyWalking](https://skywalking.apache.org/zh/2020-04-19-skywalking-quick-start/)

* 5、启动 Sentinel 分布式熔断和降级（非必须）

参考手册：[快速入门| Sentinel](https://sentinelguard.io/zh-cn/docs/quick-start.html)

* 6、启动 Seata 分布式事务（非必须）

参考手册：[快速启动 | Apache Seata](https://seata.apache.org/zh-cn/docs/user/quickstart/)

* 7、启动 Rocketmq 消息队列（非必须）

参考手册：[快速启动 | Rocketmq](https://rocketmq.apache.org/zh/docs/quickStart/01quickstart/)
...
![主界面](https://gitee.com/seaflows/picgo-bed-diagram/raw/master/image/202507221703172.png)
