...
import { getProjectDetailApi } from "@/api/lokhub-project/my-project.js"
...
export default {
  ...
  methods: {
    handleClick({ name }) {
      if (name === "任务") {
        // this.$router.push({ path: "/lokhub-project/my-task", query: { test: "test" } })
      } else if (name === "物料") {
  ...
  created() {
    ...
    getProjectDetailApi(projectId)
      ...
      .catch(() => {
        // 退回上一级路由
        this.$router.replace("/lokhub-project/my-project")
      })
  },
}
</script>
