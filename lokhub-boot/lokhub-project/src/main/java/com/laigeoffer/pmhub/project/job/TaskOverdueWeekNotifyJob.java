...
import com.laigeoffer.lokhub.project.domain.ProjectTask;
            ...
            if (projectTask != null && projectTask.getCloseTime() != null) {
                ...
                if (ChronoUnit.DAYS.between(localDate, closeDate) < 0L) {
                    if (!ProjectTaskStatusEnum.FINISHED.getStatus().equals(projectTask.getExecuteStatus())) {
                        ...
                        // 设置任务详情页
                        String url = SsoUrlUtils.ssoCreate(appid, agentid, host + path + ssoPath + URLEncoder.encode(host + "/lokhub-project/my-task/info?taskId=" + projectTaskNotify.getTaskId()));
                        ...
                        taskOvertimeRemindDTO.setLinkUrl(OAUtils.ssoCreate(host + "/lokhub-project/my-task/info?taskId=" + projectTaskNotify.getTaskId()));
