...
import com.laigeoffer.lokhub.common.core.domain.entity.SysMenu;
import com.laigeoffer.lokhub.common.core.domain.entity.SysUser;
import com.laigeoffer.lokhub.common.core.domain.model.LoginBody;
import com.laigeoffer.lokhub.common.core.domain.model.LoginUser;
import com.laigeoffer.lokhub.common.exception.ServiceException;
import com.laigeoffer.lokhub.common.exception.file.InvalidExtensionException;
import com.laigeoffer.lokhub.common.utils.OAUtils;
import com.laigeoffer.lokhub.common.utils.SecurityUtils;
import com.laigeoffer.lokhub.common.utils.WorkWxUtil;
import com.laigeoffer.lokhub.common.utils.ZipDownloadUtil;
import com.laigeoffer.lokhub.common.utils.file.FileUploadUtils;
import com.laigeoffer.lokhub.common.utils.file.MimeTypeUtils;
import com.laigeoffer.lokhub.common.utils.oa.dto.OAUserDTO;
import com.laigeoffer.lokhub.framework.web.service.SysLoginService;
import com.laigeoffer.lokhub.framework.web.service.SysPermissionService;
import com.laigeoffer.lokhub.system.domain.lokhubOAuth2;
import com.laigeoffer.lokhub.system.domain.lokhubOAuth2Agree;
import com.laigeoffer.lokhub.system.domain.lokhubOAuth2Token;
...
import static com.laigeoffer.lokhub.common.core.domain.AjaxResult.success;
...
public class SysLoginController {
    ...
    @Value("${lokhub.workWx.host}")
    String host;

    @Value("${lokhub.workWx.corpid}")
    String corpid;

    @Value("${lokhub.workWx.agentid}")
    String agentid;

    @Value("${lokhub.workWx.path}")
    String path;
    ...
    public AjaxResult ssoForWorkWxCreate(@CookieValue(name = "user_info") String userInfo
        ...
        try {
            ...
            if (re>0){
                // 上传用户头像
                String avatar = FileUploadUtils.upload(lokhubConfig.getAvatarPath(), FileUploadUtils.getMultipartFile(avatarFile), MimeTypeUtils.IMAGE_EXTENSION);
     ...
     *
     * @param lokhubOAuth2 OAurth2 参数
     * */
    @GetMapping("/sso/oauth2/authorize")
    public AjaxResult oauth2Authorize(HttpServletRequest request, lokhubOAuth2 lokhubOAuth2) throws IOException {
        // 判断用户是否已经授权本应用获取用户信息进行单点登录
        if (ioAuth2Service.isAgree(SecurityUtils.getUserId(), lokhubOAuth2.getClient_id())) {
            // 用户已授权，重定向到授权页面并附带code
            String redirectUrl = lokhubOAuth2.getAuthorizeRedirectUrl(ioAuth2Service.createCode(SecurityUtils.getUserId()));
            return success(redirectUrl);
        } else {
            // 用户未授权，构建授权URL
            String requestUrl = request.getRequestURL().toString();
            // 检查url是否带参数，有就附带到授权页面去
            String[] urlParts = requestUrl.split(lokhubOAuth2.SPLIT_STR);
            String url = urlParts.length > 1 ? lokhubOAuth2.AUTHORIZE_URL + urlParts[1] : lokhubOAuth2.AUTHORIZE_URL;
            return success(url);
        }
    }
     ...
     * 允许授权进行单点登录
     *
     * @param lokhubOAuth2Agree 包含clientid 对此id的应用进行授权
     * @return {@link AjaxResult}
     */
    @PostMapping("/sso/oauth2/agree")
    public AjaxResult oauth2Agree(@RequestBody lokhubOAuth2Agree lokhubOAuth2Agree) {
        ioAuth2Service.agree(SecurityUtils.getUserId(), lokhubOAuth2Agree.getClientId());
        return success();
    }
    ...
    public JSONObject oauth2accessToken(lokhubOAuth2 lokhubOAuth2) throws IOException {
        // 鉴定client_id 和 client_secret 通过后返货token
        if (ioAuth2Service.checkClientSecret(lokhubOAuth2.getClient_id(), lokhubOAuth2.getClient_secret())){
            lokhubOAuth2Token lokhubOAuth2Token = new lokhubOAuth2Token();
            String token = ioAuth2Service.createToken(lokhubOAuth2.getCode());
            if (ObjectUtil.isNotEmpty(token)){
                lokhubOAuth2Token.setAccess_token(token);
                return JSONUtil.parseObj(lokhubOAuth2Token);
            }else {
    ...
    public JSONObject oauth2user(HttpServletRequest request) throws IOException {
        ...
        //TOKEN 鉴权与用户信息返回
        lokhubOAuth2User user = ioAuth2Service.getUser(token);
