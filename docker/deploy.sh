...
# 启动基础环境（必须）
base(){
	docker-compose up -d lokhub-mysql lokhub-redis lokhub-nacos
}


# 启动重要服务（必须）
important(){
  docker-compose up -d lokhub-gateway lokhub-auth
}


# 启动程序模块（必须）
modules(){
	docker-compose up -d lokhub-system lokhub-project lokhub-workflow
}

# 启动不重要服务（非必须）
unimportant(){
  docker-compose up -d lokhub-gen lokhub-job lokhub-monitor
}
